...
`from_pad` is an **exact right inverse of `to_pad` on pleasure and dominance**, machine precision, not tolerance, wherever the target is reachable. Arousal is
...
`EmotionTimeline`) also preserve mood and history. Payloads are stamped
...
---
[← Emotion algebra](algebra.md) · [Home](index.md) · [CLI reference →](cli.md)
